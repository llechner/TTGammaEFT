...
bkgHists = []
for i in range( hists["total"].GetNbinsX() ):
    proc_list = []
    for p, p_dict in histDict.items():
        ...
        tmp_hist = hists[p].Clone(p+str(i))
        tmp_hist.Scale(0.)
        tmp_hist.SetBinContent(i+1, hists[p].GetBinContent(i+1))
        tmp_hist.style = styles.fillStyle( p_dict["color"], errors=False )
        for j in range( hists[p].GetNbinsX() ):
            tmp_hist.GetXaxis().SetBinLabel( j+1, crLabel[j] )
        tmp_hist.LabelsOption("v","X") #"vu" for 45 degree labels

        if i != hists[p].GetNbinsX()-2:
            tmp_hist.notInLegend = True
        else:
            tmp_hist.legendText = p_dict["texName"]
        
        if not "signal" in p or not ("SR" in crLabel[i] and not "Iso" in crLabel[i]):
            proc_list.append(tmp_hist)
        else:
            h_sig = tmp_hist

    proc_list.sort(key=lambda h: -h.Integral())
    if "SR" in crLabel[i] and not "Iso" in crLabel[i]:
        proc_list.insert(0,h_sig)
    bkgHists += proc_list
...
drawObjects = drawObjects( isData=isData, lumi_scale=lumi_scale ) + drawDivisions() + drawPTDivisions() + boxes
...
#    if hMax < 1000000. and log: heightFactor *= 5
    plotting.draw(
        ...
        plot_directory = os.path.join(plot_directory, "controlRegions", str(args.year), fit, "sort_log" if log else "sort_lin"),
